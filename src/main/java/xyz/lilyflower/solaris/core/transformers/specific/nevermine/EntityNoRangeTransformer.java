...
public class EntityNoRangeTransformer implements SolarisClassTransformer {
    void func_70097_a(TargetData data) {
        if (NevermineTransformerSettings.ALLOW_ALL || NevermineTransformerSettings.ALLOW_RANGED) {
    ...
    void solaris$metadata(ClassNode node) {
        if (NevermineTransformerSettings.ALLOW_ALL || NevermineTransformerSettings.ALLOW_RANGED) node.interfaces.remove(this.internal$transformerTarget());
    }

    @Override public String internal$transformerTarget() {
        return "net/nevermine/mob/placement/EntityNoRange";
    }
}
